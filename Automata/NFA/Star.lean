...
@[simp] theorem star_start_none : m.star.start none := rfl

@[simp] theorem star_final_none : m.star.final none := rfl
...
@[simp] theorem star_restart {xs : List α} {s t} : xs ≠ [] → m.start s → m.star.run xs (some s) t → m.star.run xs none t := by
  intro hxs hstart hrun
  match xs with
  | [] => contradiction
  | x::xs =>
    rw [m.star.run_cons] at hrun ⊢
    match hrun with
    | ⟨some u, htrans, hrun⟩ =>
      rw [star_trans_some_some] at htrans
      exists some u
      constructor
      · rw [star_trans_none_some]
        exists s
      · exact hrun
    | ⟨none, htrans, hrun⟩ =>
      rw [star_trans_some_none] at htrans
      match htrans with
      | ⟨u, htrans, hfinal⟩ =>
        exists none
        constructor
        · rw [star_trans_none_none]
          exists s, u
        · exact hrun
...
@[simp] theorem star_unstart (xs) : xs ≠ [] → m.star.run xs none none → ∃ s, m.start s ∧ m.star.run xs (some s) none := by
  ...
  | cons x xs =>
    ...
    match h with
    | ⟨some x₁, ⟨htrans, hrun⟩⟩ =>
      rw [star_trans_none_some] at htrans
      match htrans with
      | ⟨s, htrans, hfinal⟩ =>
        exists s
        constructor
        ...
        · unfold run
          ...
          exists some x₁
    | ⟨none, ⟨htrans, hrun⟩⟩ =>
      rw [star_trans_none_none] at htrans
      match htrans with
      | ⟨t, s, ⟨htrans, ⟨hstart, hfinal⟩⟩⟩ =>
        ...
        · unfold run
          ...
          exists (none)
          constructor
          · rw [star_trans_some_none]
            exists s
          · exact hrun

@[simp] theorem star_run_one {s t xs} : m.run xs s t → m.final t → xs ≠ [] → m.star.run xs (some s) none := by
...
@[simp] theorem star_run_append {xs ys s} : m.star.accept ys → m.star.run xs s none → m.star.run (xs ++ ys) s none := by
  intro hys hrun
  induction xs generalizing s with
  | nil =>
    rw [List.nil_append]
...
@[simp] theorem star_one {xs} : m.accept xs → m.star.accept xs := by
  intro h
  unfold accept at h ⊢
  simp at h ⊢
  exists none,none
...
@[simp] theorem star_append (xs ys) : m.star.accept xs → m.star.accept ys → m.star.accept (xs ++ ys) := by
  intro h h₁
  unfold accept at h h₁ ⊢
  simp at h h₁ ⊢
  exists none, none
  match h with
  | ⟨t₁, t₂, hfinal, hstart⟩ =>
...
@[simp] theorem star_sound_append {xs ys} : m.accept xs → m.star.accept ys → m.star.accept (xs ++ ys) := by
  intro h₁ h₂
  apply star_append
  · apply star_one
    exact h₁
